...
# ...
#
# Configure start and end of sub
sub_start = 1
sub_end = 109+1

# Configure experiment number to take on, which can be looped through or just one experiment
experiment_num = [5]

# Grouping all the Individual Channels (e.g., Fp1. or Fp2.) into the Channel Category (e.g., Channel Fp)
...
# Having the dict for storing average channel value at each dataframe moment
cat_avg = {"Fp": None,"Af": None,"F": None,"Ft": None,"Fc": None,"T": None,"C": None,"Tp": None,"Cp": None,"P": None,"Po": None,"O": None,"I": None}

# Sequence to loop through for each category according to the dataframe
cat_seq = ["Fc","C","Cp","Fp","Af","F","Ft","T","Tp","P","Po","O","I"]

# For each experiment listed
# Structure of files:
# Experiment 1 Dir
# |_ Subject 1 Dir  ... Subject 109 Dir
#   |_ Channel 0 Dir ... Channel 63 Dir
#       |_ Segment 0.png ... Segment 11.png
#   ...
...
for run in experiment_num:
    ...
    for i in range(sub_start, sub_end):
        ...
        os.makedirs('E:\\DeepLearning_AI_CW\\New_Experiment_'+str(run)+'_new_image_output_'+str(sub_start)+'-'+str(sub_end)+'\\subject_' + str(i) + '\\')
        #     From 0 to 64
        ...
        files = eegbci.load_data(subject, runs, "E:\\DeepLearning_AI_CW\\New_files")
        # Read raw data files where each file contains a run
        raws = [read_raw_edf(f, preload=True) for f in files]
        # Combine all loaded runs
        raw_obj = concatenate_raws(raws)
        raw_data = raw_obj.get_data()
        # Convert data to dataframe to have each channel data loaded and averaged
        ...
        for j in range(5,13):
            ...
            # Change path here for output folder
            os.makedirs('E:\\DeepLearning_AI_CW\\New_Experiment_'+str(run)+'_new_image_output_'+str(sub_start)+'-'+str(sub_end)+'\\subject_' + str(i) + '\\channel_' + str(j) + "\\")
            for segment in range(12):
                ...
                plt.show()
                # Change path here for output folder
                plt.savefig("E:\\DeepLearning_AI_CW\\New_Experiment_"+str(run)+"_new_image_output_"+str(sub_start)+"-"+str(sub_end)+"\\subject_" + str(i) + "\\channel_" + str(j) + "\\example_" + str(
